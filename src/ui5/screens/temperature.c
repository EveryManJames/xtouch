...
static void tile(lv_obj_t *parent, const char *name, lv_color_t dotc, int step,
...
void ui_temperatureScreen_screen_init(void)
{
    ...
    // 2x2 of tiles, built as two flex rows (LV_USE_GRID is off in lv_conf.h).
    ...
    char buf[8];
    tile(row1, "Nozzle", ui_theme_heat(), 5, nozzle_step, &lblNozzleCur, &lblNozzleTgt, (snprintf(buf, sizeof buf, "%d°", tgtNozzle), buf));
    tile(row1, "Bed", ui_theme_heat(), 5, bed_step, &lblBedCur, &lblBedTgt, (snprintf(buf, sizeof buf, "%d°", tgtBed), buf));
    tile(row2, "Part fan", ui_theme_chamber(), 10, part_step, &lblPartCur, &lblPartTgt, (snprintf(buf, sizeof buf, "%d%%", tgtPart), buf));
    tile(row2, "Aux fan", ui_theme_chamber(), 10, aux_step, &lblAuxCur, &lblAuxTgt, (snprintf(buf, sizeof buf, "%d%%", tgtAux), buf));
